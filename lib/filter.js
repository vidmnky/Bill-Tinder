...
const FLUFF_PATTERNS = [
  ...
  // Memorials, tributes, condolences
  {
    test: (title) => /\b(memorial|memorialize|in memory of|tribute|condolence|sympathy|death of|life of|mourning)\b/i.test(title),
    reason: 'memorial_or_tribute',
  },
  // Building and post office namings
  {
    test: (title) => /\b(designat|nam)(e|es|ed|ing)\b.{0,40}\b(building|post office|courthouse|facility|bridge|highway|road|street|park|center|memorial|room|plaza|interchange)/i.test(title),
  ...
  // Proclamations and declarations of special days
  {
    test: (title) => /\b(proclaim|declaring|declare)\b.{0,60}\b(day|week|month|year)\b/i.test(title),
    reason: 'proclamation',
  },
  // Coins and medals
  {
  ...
  // Resolutions that are purely ceremonial (joint/simple resolutions with ceremonial language)
  {
    test: (title) => /\b(resolution|urging|expressing)\b.{0,40}\b(support for|opposition to|recognizing|honoring|condemning|praising|saluting|welcoming)\b/i.test(title),
    reason: 'ceremonial_resolution',
  },
];
