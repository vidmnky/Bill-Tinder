...
export const MONTHLY_HARD_LIMIT = 30000;    // LegiScan's actual limit
export const MONTHLY_SOFT_LIMIT = 1000;     // Our self-imposed routine cutoff
export const MONTHLY_EMERGENCY_STOP = 2000; // Something is very wrong if we hit this
export const MIN_MS_BETWEEN_CALLS = 200;    // Rate limit: 5 calls/second max
 ...
 */
export function checkBudget(monthlyRequestCount) {
 ...
 */
export function getChangedDatasets(remoteDatasets, storedHashes) {
  return remoteDatasets.filter(ds => {
    const storedHash = storedHashes.get(ds.session_id);
 ...
 */
export function preFlightCheck(monthlyRequestCount, datasetsNeeded) {
